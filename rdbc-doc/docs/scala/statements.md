...
def findUsersStmt(age: Int): Future[ExecutableStatement] = {
   conn.statement(sql"select * from users where age = $age")
}
```
...
```scala
def findUsersStmt(age: Int): Future[ExecutableStatement] = {   
    conn.statement("select * from users where age = ?")
      .map(stmt => stmt.bindByIndex(age))   
}
```
...
In the above example, only `$name` is a statement parameter, `#$table` is going
to be simply replaced by `table` method parameter value.
