...
class DoubleConverterSpec
  extends RdbcTypeconvSpec
    with Inside {

  private val converter = DoubleConverter

  "DoubleConverter" should {
    "convert BigDecimal" in {
      val bd = BigDecimal("42.2")
      converter.fromAny(bd) shouldEqual 42.2d
    }

    "convert Double" in {
      val d = 42.2d
      converter.fromAny(d) shouldEqual 42.2d
    }

    "convert Float" in {
      val f = 42.2f
      converter.fromAny(f) shouldEqual 42.2d
    }

    "convert Long" in {
      val l = 42L
      converter.fromAny(l) shouldEqual 42.0d
    }

    "convert Int" in {
      val i = 42
      converter.fromAny(i) shouldEqual 42.0d
    }

    "convert Short" in {
      val s = 42.toShort
      converter.fromAny(s) shouldEqual 42.0d
    }

    "convert Byte" in {
      val b = 42.toByte
      converter.fromAny(b) shouldEqual 42.0d
    }

    "convert finite DecimalNumber" in {
      val bd = BigDecimal("42.2")
      val s = DecimalNumber.Val(bd)
      converter.fromAny(s) shouldEqual 42.2d
    }
    ...
    "convert NaN DecimalNumber" in {
      ...
      inside(converter.fromAny(s)) { case d: Double =>
        d.isNaN === true
      }
    }
    ...
    "convert NaN Double" in {
      val d = Double.NaN
      inside(converter.fromAny(d)) { case d: Double =>
        d.isNaN === true
      }
    }
    ...
    "convert NaN Float" in {
      val f = Float.NaN
      inside(converter.fromAny(f)) { case d: Double =>
        d.isNaN === true
      }
    }
    ...
    "fail for string" in {
      ...
      ex.targetType shouldEqual classOf[Double]
    }
  }
}
