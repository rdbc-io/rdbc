...
class IntConverterSpec
  ...
  "IntConverter" should {
    "convert BigDecimal" in {
      ...
      converter.fromAny(bd) shouldBe 42
    }

    "convert Double" in {
      val d = 42.2d
      converter.fromAny(d) shouldBe 42
    }

    "convert Float" in {
      val f = 42.2f
      converter.fromAny(f) shouldBe 42
    }

    "convert Long" in {
      val l = 42L
      converter.fromAny(l) shouldBe 42
    }

    "convert Int" in {
      val i = 42
      converter.fromAny(i) shouldBe 42
    }

    "convert Short" in {
      val s = 42.toShort
      converter.fromAny(s) shouldBe 42
    }

    "convert Byte" in {
      val b = 42.toByte
      converter.fromAny(b) shouldBe 42
    }
    ...
    "fail for -inf DecimalNumber" in {
      ...
      ex.value shouldBe s
      ex.targetType shouldBe classOf[Int]
    }
